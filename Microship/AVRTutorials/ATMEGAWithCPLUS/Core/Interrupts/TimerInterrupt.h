...
class TimerInterrupt
{
	...
	//Interrupt Timer 
	INTERRUPT_TIMER _timer; 
	
	//Normal , Compare ,Input Capture
	INTERRUPT_MODE _timerMode;
	
	
	public:
	
	//Timer Interrupt Without Enable Interrupt
	TimerInterrupt(INTERRUPT_TIMER timer,INTERRUPT_MODE mode);
	
	//Timer Interrupt With Enable Interrupt
	TimerInterrupt(INTERRUPT_TIMER timer,INTERRUPT_MODE mode,void(* functionPtr)());
	...
	//Disable Timer Interrupt For Active Timer,Mode
